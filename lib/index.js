...
function isPermissionErrno(err) {
	const code = err?.code;
...
function toFilesystemError(err, path) {
	if (isPermissionErrno(err)) return new SkillManagerError("filesystem-permission", `permission denied on ${path}`, { path });
...
var SkillRoot = class {
	...
	*/
	async ensureRootExists() {
		if (!await tryLstat(this.path)) await wrapFs(mkdir(this.path, { recursive: true }), this.path);
	}
	/**
...
function splitDownloadId(id) {
	...
	if ([
		...
		repo,
		slug
	].some((segment) => segment === "." || segment === "..")) return null;
	return {
		owner,
		repo,
		slug
...
async function installSkill(deps, request, signal) {
	...
	if (loaded.status === "corrupt") throw new InstallError("manifest-corruption", `cannot install: the manifest is corrupt (${loaded.corruption?.reason ?? "unknown"})`);
...
function toInstallError(err) {
	...
	const message = err instanceof Error ? err.message : String(err);
	if (isPermissionErrno(err)) return new InstallError("filesystem-permission", message, { cause: err });
...
async function recoverInterruptedSwap(root) {
	...
	for (const entry of entries) {
		...
		if (await root.classifySkill(slug) !== "missing") await root.removeBackup(slug).catch(() => {});
...
var UpdateManager = class {
	...
	*/
	async checkUpdates() {
		return { updates: (await this.computeSkillStates({ recover: true })).map(({ info }) => info) };
	}
	/**
	...
	*/
	async list() {
		return { skills: (await this.computeSkillStates({ recover: false })).map(({ slug, id, entry, info }) => ({
			slug,
			source: entry.source,
			id,
			remoteSourceHash: entry.remoteSourceHash,
	...
	*/
	async computeSkillStates(options) {
		if (options.recover) await recoverInterruptedSwap(this.root);
...
function readSearchQuery(payload) {
	if (typeof payload !== "object" || payload === null || Array.isArray(payload)) throw new SkillManagerError("invalid-request", "search requires a payload object");
	const query = payload.query;
	if (typeof query !== "string") throw new SkillManagerError("invalid-request", "search requires a `query` string");
	return query;
}
function readDescribeId(payload) {
	if (typeof payload !== "object" || payload === null || Array.isArray(payload)) throw new SkillManagerError("invalid-request", "describe requires a payload object");
	const id = payload.id;
	if (typeof id !== "string" || id.trim() === "") throw new SkillManagerError("invalid-request", "describe requires a non-empty `id` string");
	return id;
}
/**
...
*/
async function uninstallSkill(root, store, input) {
	...
	if (!entry) {
		if (load.dropped.includes(id)) {
			...
			await persistManifest(store, load.manifest, `skill "${id}" is already uninstalled but its manifest entry could not be removed`);
			return { ok: true };
		}
	...
	if (await hasLocalDrift(root, id, entry.localContentHash) && input.discardLocalChanges !== true) throw new SkillManagerError("local-modification-conflict", `skill "${id}" has local changes that will be discarded`, { id });
	...
	await persistManifest(store, withoutEntry(load.manifest, id), "removed the skill directory but failed to update the manifest");
	return { ok: true };
}
/** Recompute the local-content hash and compare to the recorded value. */
async function hasLocalDrift(root, id, recorded) {
	const kind = await root.classifySkill(id);
	if (kind === "missing") return false;
	if (kind !== "directory") return true;
	return await computeLocalContentHash(root.skillDir(id)) !== recorded;
}
async function loadManifest(root, store) {
	try {
...
async function persistManifest(store, manifest, failureMessage) {
	try {
...
function withoutEntry(manifest, id) {
...
var SkillManagerService = class {
	...
	*/
	async search(query, signal) {
		return {
			results: await this.deps.client.search(query, { signal }),
			complete: true
		};
	}
	/**
	...
	*/
	async describe(id, signal) {
		return { description: await this.deps.client.getDescription(id, { signal }) };
	}
