...
*/
async function readSkillFiles(root, slug) {
	...
	while (pending.length > 0) {
		...
		for (const entry of entries) {
			...
			if (entry.isSymbolicLink()) files.push({
				path: relPath,
				contents: await readlink(absPath)
			});
			else if (entry.isDirectory()) pending.push({
				abs: absPath,
				rel: relPath
			});
...
function assertSnapshotSafe(snapshot) {
	if (!Array.isArray(snapshot.files) || snapshot.files.length === 0) throw new UpdateError("invalid-snapshot", "snapshot contains no files");
	for (const file of snapshot.files) assertRelativePath(file.path);
	const skillMd = snapshot.files.find((file) => file.path === "SKILL.md");
	if (!skillMd) throw new UpdateError("invalid-snapshot", "snapshot is missing SKILL.md");
	const metadata = extractFrontmatterMetadata(skillMd.contents);
	if (!metadata.name || !metadata.description) throw new UpdateError("invalid-snapshot", "SKILL.md frontmatter must declare both a name and a description");
}
//#endregion
...
*/
async function swapStaged(root, slug, options = {}) {
	const rename$1 = options.rename ?? rename;
	...
	await rename$1(to, backup);
	try {
		await rename$1(from, to);
	} catch (err) {
		await rename$1(backup, to).catch(() => {});
		throw err;
	}
	await rm(backup, {
		recursive: true,
...
async function recoverInterruptedSwap(root) {
	...
	for (const entry of entries) {
		...
		if (await pathExists(to)) await rm(backup, {
			recursive: true,
			force: true
		}).catch(() => {});
		else {
...
var UpdateManager = class {
	...
	*/
	async update(input) {
		...
		const drifted = localContentHash(await readSkillFiles(this.root, slug)) !== entry.localContentHash;
		...
		try {
			...
			throw new UpdateError("update-partial-failure", `skill files updated but manifest write failed: ${err instanceof Error ? err.message : String(err)}`, {});
		}
		return {
			slug,
			source: entry.source,
...
async function writeSnapshotFiles(root, staging, files) {
